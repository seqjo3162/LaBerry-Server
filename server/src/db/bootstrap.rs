...
pub async fn ensure_global_server(db: &PgPool) -> anyhow::Result<()> {
    let exists: Option<i64> = sqlx::query_scalar(sqlx::AssertSqlSafe(format!(
    ...
    let system_user_id: i64 = match sqlx::query_scalar(sqlx::AssertSqlSafe(format!(
        "SELECT id FROM {} WHERE username = '__system__' LIMIT 1",
...
pub async fn add_user_to_global_server(db: &PgPool, user_id: i64) -> anyhow::Result<()> {
    let server_exists: Option<i64> = sqlx::query_scalar(sqlx::AssertSqlSafe(format!(
        "SELECT id FROM {} WHERE id = $1",
        ServerIden::Table.to_string()
    )))
    .bind(GLOBAL_SERVER_ID)
    ...
    let user_exists: Option<i64> = sqlx::query_scalar(sqlx::AssertSqlSafe(format!(
        "SELECT id FROM {} WHERE id = $1",
        UserIden::Table.to_string()
    )))
    .bind(user_id)

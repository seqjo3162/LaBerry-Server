...
use std::{collections::HashSet, io::Cursor, path::{Path as FsPath, PathBuf}};
...
pub(super) fn thumb_path_for(stored_filename: &str) -> PathBuf {
...
fn old_enough_to_cleanup(path: &FsPath) -> bool {
...
pub(crate) async fn cleanup_orphan_storage_files(st: &AppState) {
    ...
    if let Ok(mut dir) = fs::read_dir(&storage_dir).await {
        while let Ok(Some(entry)) = dir.next_entry().await {
            ...
            if !active_paths.contains(&key) && old_enough_to_cleanup(&path) {
    ...
    if let Ok(mut dir) = fs::read_dir(&thumbs_dir).await {
        while let Ok(Some(entry)) = dir.next_entry().await {
            ...
            let key = path.to_string_lossy().to_string();
            if !active_thumbs.contains(&key) && old_enough_to_cleanup(&path) {
                let _ = fs::remove_file(path).await;
            }
        }
    }
...
pub(crate) async fn cleanup_expired_files(st: &AppState) {
    let rows: Vec<super::ExpiredFileRow> = sqlx::query_as(
        r#"
...
pub(crate) async fn upload_file(
    ...
    if let Err(e) = fs::create_dir_all(&storage_dir).await {
        ...
        tracing::error!("[FILES] storage_create_failed: {}", msg);
    ...
    let r = match r {
        ...
        Err(e) => {
            ...
            tracing::error!("[FILES] upload db_insert_failed: {}", err_text);
            let _ = fs::remove_file(&path).await;
            let _ = fs::remove_file(thumb_path_for(&stored_filename)).await;
